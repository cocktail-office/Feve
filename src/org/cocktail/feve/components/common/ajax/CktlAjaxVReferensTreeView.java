 ...
 */
public class CktlAjaxVReferensTreeView extends ATreeViewComponent {
	...
	private CktlAjaxVReferensTreeViewCtrl ctrl;
	private EOVReferens selection;
	private String filtreTexte;

	public CktlAjaxVReferensTreeView(WOContext context) {
		super(context);
	}
	...
	private void resetTree() {
		ctrl().rootVReferens().setChildren(null);
		EOVReferens formation = selection();
		if (formation != null)
			ctrl().selectObjectInTree(ctrl().rootVReferens(), formation);
	}

	public CktlAjaxVReferensTreeViewCtrl ctrl() {
		if (ctrl == null) {
			if (hasBinding(CTRL_BDG)) {
				ctrl = (CktlAjaxVReferensTreeViewCtrl) valueForBinding(CTRL_BDG);
			}
			if (ctrl == null) {
				ctrl = new CktlAjaxVReferensTreeViewCtrl(this);
			}
		}
		return ctrl;
	}
...
//			ctrl().setRootFormation(null);
	...
	public String cssClassForSelectedLink() {
		...
		if (hasBinding(CSS_SELECTED_GROUPE_BDG) &&
				selection() != null && selection().equals(ctrl().unVReferens().object())) {
	...
	private EOQualifier getTexteQualifier() {
		if (filtreTexte != null)
			return null;//XXX ERXQ.contains(EOVReferens.LIBELLE_DESCENDANCE_KEY, filtreTexte);
		else
			return null;
	}
	...
	public EOVReferens treeRootObject() {
		return (EOVReferens) valueForBinding(TREE_ROOT_OBJECT_BDG);
	}
	...
	public String selectionOnClickBefore() {
		if (hasBinding(CONFIRM_BEFORE_SELECTION_BDG)) {
			if (((Boolean) valueForBinding(CONFIRM_BEFORE_SELECTION_BDG)).booleanValue()) {
				return ("confirm('Voulez-vous selectionner la formation " + jsEncode(ctrl.unVReferensLibelle()) + " ?')");
			}
		}
		return null;
	}
	 ...
	 */
	public void setCtrl(CktlAjaxVReferensTreeViewCtrl ctrl) {
		this.ctrl = ctrl;
	}

	public EOVReferens selection() {
		if (selection != valueForBinding(SELECTION_BDG))
			selection = (EOVReferens) valueForBinding(SELECTION_BDG);
		return selection;
	}
	...
	public void setSelection(EOVReferens selection) {
		...
		setValueForBinding(selection, CktlAjaxVReferensTreeView.SELECTION_BDG);
	}
